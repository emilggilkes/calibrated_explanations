...
class IntervalRegressor:
    ...
    def __init__(self, calibrated_explainer):
        ...
        self.model = self
        self.cal_y_hat = self.ce.model.predict(
            self.ce.cal_X
        )  # can be calculated through calibrated_explainer
        self.residual_cal = (
            self.ce.cal_y - self.cal_y_hat
        )  # can be calculated through calibrated_explainer
    ...
    def predict_probability(self, test_X, y_threshold, bins=None):
        ...
        if np.isscalar(self.y_threshold):
            ...
            if bins is not None:
                assert (
                    self.ce.bins is not None
                ), "Calibration bins must be assigned when test bins are submitted."
            self.compute_proba_cal(self.y_threshold)
            proba, low, high = self.venn_abers.predict_proba(
                test_X, output_interval=True, bins=bins
            )
        ...
        for i, _ in enumerate(proba):
            ...
            p, low, high = self.venn_abers.predict_proba(
                test_X[i, :].reshape(1, -1), output_interval=True, bins=bins
            )
            proba[i] = p[:, 1]
    ...
    def predict_uncertainty(self, test_X, low_high_percentiles, bins=None):
        ...
        sigma_test = self.ce._get_sigma_test(
            X=test_X
        ...
        return (
            ...
            (
                interval[:, 0]
                if low_high_percentiles[0] != -np.inf
                else np.array([np.min(self.ce.cal_y)])
            ),
            (
                interval[:, 2]
                if low_high_percentiles[1] != np.inf
                else np.array([np.max(self.ce.cal_y)])
            ),
            None,
        )
    ...
    def predict_proba(self, test_X, bins=None):
        ...
        proba = self.cps.predict(
            y_hat=test_y_hat, sigmas=sigma_test, y=self.current_y_threshold, bins=bins
        )
        # return np.array([[1 - proba[i], proba[i]] for i in range(len(proba))])
        return np.array([[proba[i], 1 - proba[i]] for i in range(len(proba))])
    ...
    def compute_proba_cal(self, y_threshold: float):
        ...
        if self.ce.difficulty_estimator is not None:
            sigmas = self.ce._get_sigma_test(
                self.ce.cal_X
            )  # pylint: disable=protected-access
            proba = self.cps.predict(
                y_hat=self.cal_y_hat, y=y_threshold, sigmas=sigmas, bins=self.ce.bins
            )
            self.proba_cal = np.array(
                [[1 - proba[i], proba[i]] for i in range(len(proba))]
            )
        else:
            cps = crepes.ConformalPredictiveSystem()
            ...
            for i, _ in enumerate(self.residual_cal):
                ...
                sigma_cal = self.ce._get_sigma_test(
                    self.ce.cal_X[idx, :]
                )  # pylint: disable=protected-access
                bin_cal = self.ce.bins[idx] if self.ce.bins is not None else None
                bin_i = [self.ce.bins[i]] if self.ce.bins is not None else None
                cps.fit(
                    residuals=self.residual_cal[idx], sigmas=sigma_cal, bins=bin_cal
                )
                sigma_i = self.ce._get_sigma_test(
                    self.ce.cal_X[i, :].reshape(1, -1)
                )  # pylint: disable=protected-access
                # self.proba_cal[i, 1] = cps.predict(
                #     y_hat=[self.cal_y_hat[i]], y=y_threshold, sigmas=sigma_i, bins=bin_i
                # )
                self.proba_cal[i, 0] = cps.predict(
                    y_hat=[self.cal_y_hat[i]], y=y_threshold, sigmas=sigma_i, bins=bin_i
                )
                # self.proba_cal[i, 0] = 1 - self.proba_cal[i, 1]
                self.proba_cal[i, 1] = 1 - self.proba_cal[i, 0]
        self.venn_abers = VennAbers(
            self.proba_cal,
            (self.ce.cal_y > y_threshold).astype(int),
            self,
            bins=self.ce.bins,
        )
